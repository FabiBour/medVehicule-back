...
authorizationsRouter.get(
  ...
  requireAuth,
  async (req, res) => {
    const where = req.user.role === 'admin' ? {} : { user: { hospitalId: req.user.hospitalId } };
...
authorizationsRouter.get(
  ...
  async (req, res) => {
    ...
    if (user.hospitalId !== req.user.hospitalId && req.user.role !== 'admin') {
...
authorizationsRouter.post(
  ...
  async (req, res) => {
    ...
    if (auth.user.hospitalId !== req.user.hospitalId && req.user.role !== 'admin') {
      return res.status(403).json({ error: 'Accès refusé' });
    }
